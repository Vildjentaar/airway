# llm_engine.py — Bug Fix Summary

All 10 issues from the audit have been addressed in a single edit pass. Syntax verified.
...
| # | Issue | Fix |
|---|-------|-----|
| **1** | `active_tools` computed once, not per turn | Moved `_select_active_tools()` inside the `for turn` loop so it reflects current `flight_data`/`report_data` state after mutations |
| **2** | Stale `last_error` survives successful recovery | `last_error = None` when the `else` branch produces a clean assistant reply |
| **3** | Orphan cleanup checks "any", not "all" | Changed to `found_ids != tool_call_ids` — drops the assistant message unless **every** `tool_call_id` has a matching tool response |
| **4** | `max_turns` exhaustion produces no reply | Already had a `for/else` fallback — confirmed it appends a visible assistant message and sets `last_error` |
| **5** | Assistant-merge ignores prev's `tool_calls` | Added `and not prev.get("tool_calls")` guard so we never merge free text into a tool-call message |
| **6** | Duplicate check only compares first segment | Now computes a tuple of `(flight_number, departure_date)` for **all** segments and compares the full tuple |
| **7** | No cross-segment chronological validation | Each segment's departure date must be ≥ the previous segment's; returns a clear error if violated |
| **8** | Negative passenger counts allowed | Added `adult_count < 0 or child_count < 0 or baby_count < 0` guard before the sum check |
| **9** | Dead `hidden` placeholder branch | Removed the `len(messages) <= 2 and messages[-1].get("hidden")` branch from `_select_active_tools` — it was never wired up |
| **10** | `looks_like_text_tool_call` leaves a dead-end | Now appends an assistant message ("Could you please repeat your last request?") so the conversation continues |

## Additional Reconciliations

- **`success` vs `error`**: Changed `"success": True` to `"success": last_error is None` so the two fields are always consistent.
- **Dead `db_check_capacity` import**: Removed the `try/except ImportError` block — it was never referenced.

## Minor Notes — Status

| Note | Status |
|------|--------|
| `is_valid_flight_data` never called in this file | **Not dead code** — imported and used by `app.py` and `UI/export.py` |
| `check_capacity` double-counting risk | Not addressed — requires broader session-level cart awareness beyond this file |
